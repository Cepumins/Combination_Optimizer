...
def pair_replacement(pair_data, min_float, max_float, data):
    ...
    data_pairs.set_index(['ID_x', 'ID_y'], inplace=True)

    pair_data_IDs = set(pair_data['ID'].values)
    ...
    while improvement_found:
        ...
        for idx, (row1, row2) in enumerate(itertools.combinations(pair_data.itertuples(), 2)):
            k1, k2 = row1.ID, row2.ID
            pairs_not_including_k1_k2 = data_pairs.loc[~data_pairs.index.isin(pair_data_IDs.union({k1, k2}), level=0)]
            pairs_less_than_current_price = pairs_not_including_k1_k2[pairs_not_including_k1_k2['total_price'] < row1.formattedPrice + row2.formattedPrice]

            if not pairs_less_than_current_price.empty:
                best_pair = pairs_less_than_current_price.loc[pairs_less_than_current_price['total_price'].idxmin()]
        ...
        if best_replacement:
            ...
            pair_data_IDs.remove(k1)
            pair_data_IDs.remove(k2)
            pair_data_IDs.add(best_pair['ID_x'])
            pair_data_IDs.add(best_pair['ID_y'])
            ...
            print(f"Old price: {old_price} New price: {new_price} Decrease: {old_price - new_price}")
    return pair_data
