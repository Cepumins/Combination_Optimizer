...
def adjust_float_to_range(float_data, min_float, max_float, data):
    ...
    while not within_range(float_data['Float'].mean()):
        ...
        for _, row in float_data.iterrows():
            filtered_data = data[(data['Price'] * 1.25<= row['Price']) & (~data['Coll_ID'].isin(float_data['Coll_ID']))]
...
def main():
    ...
    with open(filename, 'r') as file: # Load the JSON data from the file
        ...
        for item in data:
            ...
            if item[0] == collection:
                if item[1] == "Covert":  # Check if the item is Covert
                    for wear_info in item[3]:  # item[3] contains the wear data
                        ...
                        expected_value = wear_details['split']['10']['ev']  # Assuming you want the 'ev' under 'split' '10'
    ...
    for wear_outcome, wear_data in combined_wear_data.items(): # Iterate through all wear outcomes
        ...
        # end the timer
        wear_ending_time = time.time()
        # Calculate and print the time required
        time_for_wear_outcome = wear_ending_time - wear_starting_time
        
        print(f"Time required for wear {wear_outcome}: {int(time_for_wear_outcome // 60):02d}:{int(time_for_wear_outcome % 60):02d}:{int((time_for_wear_outcome % 1) * 1000):03d}")
