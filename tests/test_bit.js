...
async function simulateMouseMovements(page, moveCount, width, height) {  // Simulate irregular mouse movements by randomly moving the mouse around within the viewport dimensions
    ...
    for (let i = 0; i < timesToMove+1; i++) {
        ...
        cursorX = (getRandomWeightedTowardsCenter() - 0.5) * width + width * 0.5;
        //const y = Math.random() * height; 
        //const y = (Math.pow(Math.random(), 3) - 0.5) * height + height*0.4 // Random y-coordinate within the viewport height
        cursorY = (getRandomWeightedTowardsCenter() - 0.5) * height + height * 0.4;
        await page.mouse.move(cursorX, cursorY, { steps: 10 }); // Move mouse to (x, y) with intermediate steps for smoother movement
        //console.log(`x: ${x}, dy: ${y}`);
        //await moveCustomCursor(page, x, y);
        await waitForRandomTimeout(page, 0, 100);
    }
    console.log(`Returning - cursorX: ${cursorX}, cursorY: ${cursorY}`);
...
async function scrapeCombinedItems(page, source, exchangeRatio, width, height, totalItems = null, minX = 0, minY = 0) {
    ...
    if (source === 'Port') {
        ...
        //priceSelector = 'div.market-items .item-price.flex > div.price > div.amount:first-child';
    ...
    let {cursorX, cursorY} = await simulateMouseMovements(page, 2, width, height);
    ...
    while (true) {
        try {
            ...
            //console.log(`Amount of floats: ${floats.length}`);
            //console.log(`Amount of prices: ${prices.length}`);
            //console.log(`Amount of itemAlts: ${itemAlts.length}`);
...
(async () => {
    ...
    try {
        ...
        console.error(`Error scraping Bit for ${collection} and ${rarity}: ${error}`);
    }
