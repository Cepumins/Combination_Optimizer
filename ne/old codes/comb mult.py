...
def adjust_float_to_range(float_data, min_float, max_float, data, combo, print_f):
    ...
    while not within_range(float_data['Float'].mean()):
        ...
        for _, row in float_data.iterrows():
            ...
            #mask = (filtered_data['Price'] * 1.25 <= row['Price'])
    ...
    for rarity in rarities:
        ...
        for r in range(1, total_num_collections + 1):
            ...
            #for collection in collections:
                ...
                print(f"Elapsed Time for All functions: {int(functions_total_time // 60):02d}:{int(functions_total_time % 60):02d}:{int((functions_total_time % 1) * 1000):03d}")
                if total_num_collections > 1:
                    coll_elapsed_time = time.time() - coll_start_time # end the timer and print the elapsed time
                    print(f"Elapsed Time for {list(combo)}: {int(coll_elapsed_time // 60):02d}:{int(coll_elapsed_time % 60):02d}:{int((coll_elapsed_time % 1) * 1000):03d}")
